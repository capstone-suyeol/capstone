...
const NewMeetingModal = ({ onClose }) => {
    ...
    const userId = localStorage.getItem('user_id'); // Current user's ID

    // Handle form submission
    const handleSubmit = async (event) => {
        event.preventDefault();  // Prevent default form submission
        try {
            const response = await axios.post('https://172.20.10.3/api/meetings/', {
                title: title,
...
const AttendMeetingModal = ({ onClose, onConfirm }) => {
    ...
    const participate = async (event) => {
        event.preventDefault();  // Prevent default form submission
        try {
            const response = await axios.post(`https://172.20.10.3/api/participants/`, {
                meeting: meetingId,
                user: userId,
            });
